...
const Dashboard = ({ userId }) => {
  ...
  const fetchEvents = async () => {
    const response = await axios.post(
      `${BaseUrl}get-events`,
      { status: "active", uid: userId, con: 1 },
      {
        headers: {
  ...
  useEffect(() => {
    if (dailys && dailys.length > 0) {
      ...
      // Filter events based on date and days condition
      const currentDate = new Date();
      const filteredData = sortedData.filter((event) => {
        const eventDate = new Date(event.date.split("-").reverse().join("-"));
        const timeDifference = eventDate - currentDate;
        return eventDate >= currentDate && timeDifference <= (event.days * 24 * 60 * 60 * 1000);
      });
  
      setSortedDailys(filteredData);
    }
  }, [dailys, sortOption]);
  ...
  return (
    <div className="w-full max-w-[1400px] mx-auto flex-grow min-h-[80vh] p-4 md:p-8 relative max-lg:py-10  z-20">
